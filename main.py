...
# ---------------------------------------------------------
# [6. 세 번째 그래프: TOP 5 영화 다중 선 그래프]
# ---------------------------------------------------------

st.divider() # 구역을 나누는 가로줄 긋기

st.subheader("📈 3. 흥행 TOP 5 영화 누적관객수 비교")

# 1. 누적관객수가 가장 높은 5개 영화 이름 뽑기
# 위에서 만들었던 movie_max_audience(영화별 최대 누적관객수 내림차순 정렬)에서 상위 5개만 가져옵니다.
top5_movies = movie_max_audience.head(5).index.tolist()

# 2. 전체 데이터(df)에서 TOP 5 영화에 해당하는 데이터만 걸러내기
# isin() 함수는 데이터가 리스트 안에 포함되어 있는지 확인해줍니다.
top5_df = df[df['영화명'].isin(top5_movies)]

# 3. 다중 선 그래프 그리기
# color='영화명' 옵션을 넣으면 영화마다 다른 색상으로 선이 그려지고, 우측에 범례가 자동으로 생깁니다.
fig3 = px.line(top5_df, 
               x='기준일자', 
               y='누적관객수', 
               color='영화명', # 영화별로 색상과 선 나누기
               markers=False) # 데이터가 많아 선이 지저분해 보이지 않도록 마커(점)는 뺐습니다.

# 4. 스트림릿 화면에 만들어진 다중 선 그래프 띄우기
st.plotly_chart(fig3, use_container_width=True)

# 5. 그래프 아래에 분석 내용을 적을 자리 만들기
st.info("💡 이 그래프로 알 수 있는 것: (예: 어떤 영화가 개봉 초반 흥행 속도가 가장 빨랐는지, 특정 영화가 다른 영화를 언제 역전했는지 그 추이를 한눈에 비교할 수 있습니다.)")
